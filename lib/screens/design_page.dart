...
class DesignService {
  ...
  static Future<List<Project>> fetchDesignProjects() async {
    try {
      final response = await http.get(Uri.parse('$baseUrl/projects/?category__slug=design'));
      ...
      debugPrint("Erreur API Design: $e");
      throw Exception("Serveur Django introuvable.");
    }
  }
}
...
class _DesignPageState extends State<DesignPage> {
  ...
  void initState() {
    super.initState();
    _futureProjects = DesignService.fetchDesignProjects();
  }
  ...
  Widget build(BuildContext context) {
    ...
    final String cvUrl = lang.isFrench
        ? "https://drive.google.com/file/d/1UQz7Pr1RpuLgU3TZrCiYGUA5XnrtfrrJ/view?usp=sharing"
        : "https://drive.google.com/file/d/1EH8_UjkEhdYHlA2J9-LJEYvejqNqv4Ix/view?usp=sharing";
                ...
                child: Center(
                  child: ConstrainedBox(
                    ...
                    child: Column(children: [
                      _buildToolsSection(context, lang, isMobile).animate().fadeIn(duration: 800.ms).moveY(begin: 30, end: 0),
  ...
  Widget _navBtn({required VoidCallback onPressed, required IconData icon, required String label}) {
  ...
  Widget _buildToolsSection(BuildContext context, LanguageProvider lang, bool isMobile) {
    final tools = {
      'Photoshop': 'photoshop', 'Illustrator': 'illustrator', 'Figma': 'figma',
      'Premiere Pro': 'premiere', 'After Effects': 'after', 'CapCut': 'capcut'
    };
    return Container(
      ...
      child: Column(children: [
        _sectionTitle(lang.t("OUTILS & CRÉATION", "TOOLS & CREATIVE SUITE")),
        const SizedBox(height: 40),
        Wrap(
          spacing: isMobile ? 25 : 50,
          runSpacing: isMobile ? 25 : 40,
          alignment: WrapAlignment.center,
          children: tools.entries.map((tool) => _buildToolIcon(tool.key, tool.value, isMobile)).toList(),
        ),
      ]),
    );
  }

  Widget _buildToolIcon(String name, String iconName, bool isMobile) {
    ...
    return Column(children: [
      Container(
        ...
        child: Image.asset(
            ...
            errorBuilder: (c, e, s) => const Icon(Icons.brush, color: Colors.white10)
        ),
      ),
      const SizedBox(height: 12),
      Text(name, style: const TextStyle(color: Colors.white70, fontSize: 10, fontWeight: FontWeight.w600, letterSpacing: 1.2, decoration: TextDecoration.none)),
    ]);
  }

  Widget _buildProjectsSection(BuildContext context, LanguageProvider lang, bool isMobile) {
    return Column(children: [
      _sectionTitle(lang.t("PORTFOLIO CRÉATIF", "CREATIVE PORTFOLIO")),
      ...
      FutureBuilder<List<Project>>(
        ...
        builder: (context, snapshot) {
          ...
          if (projects.isEmpty) {
            return Text(
                lang.t("Aucun projet design trouvé.", "No design projects found."),
          ...
          return GridView.builder(
            ...
            itemBuilder: (context, index) => _buildDesignCard(projects[index], lang)
  ...
  Widget _buildDesignCard(Project project, LanguageProvider lang) {
    return MouseRegion(
              ...
              Expanded(
                ...
                child: project.cardImage != null
                  ...
                  errorBuilder: (c, e, s) => const Center(child: Icon(Icons.broken_image, color: Colors.white10)),
                    ...
                    : Container(color: Colors.white10, width: double.infinity, child: const Icon(Icons.brush, size: 50, color: Colors.blueAccent)),
              ),
              Expanded(
                flex: 5,
                child: Padding(
  ...
  Widget _buildErrorWidget(LanguageProvider lang) {
    return Column(children: [
      ...
      TextButton(
          onPressed: () => setState(() { _futureProjects = DesignService.fetchDesignProjects(); }),
